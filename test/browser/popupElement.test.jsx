...
beforeEach(async () => {
  ...
  await waitTimeout(5); // timeout required because of debounceFilters
});

describe("popupElement", () => {
  test("clickOnLabel with input - single event", async () => {
    await page.click("label");
    await waitTimeout(320); // timeout required because of debounceFilters
    ...
    await waitTimeout(310); // wait for animation
    t = await page.evaluate(() => ({ ...t, html: testEl.outerHTML, isOpened: testEl.$isOpened }));
    ...
    await page.click("label");
    await waitTimeout(1); // timeout required because of debounceFilters
    t = await page.evaluate(() => ({ ...t, html: testEl.outerHTML, isOpened: testEl.$isOpened }));
    expect(t.isOpened).toBe(true);
    await page.click("label"); // click again should hide
    t = await page.evaluate(() => ({ ...t, html: testEl.outerHTML, isOpened: testEl.$isOpened }));
  ...
  test("openCase: click & focus", async () => {
    ...
    await waitTimeout(301); // timeout required because of animation
    t = await page.evaluate(() => ({ ...t, html: testEl.outerHTML, isOpened: testEl.$isOpened }));
    expect(t.isOpened).toBe(true);
    expect(t.gotShow).toBe(1);
    expect(t.gotHide).toBe(0);
  ...
  test("openCase: hover & click & focus", async () => {
    ...
    await page.click("body"); // otherwise hover doesn't work
    await waitTimeout(1); // timeout required because of debounceFilters
    await page.hover("input");
    let t = await page.evaluate(() => ({ ...t, html: testEl.outerHTML, isOpened: testEl.$isOpened }));
    expect(t.isOpened).toBe(false); // because of timeout
    await waitTimeout(300); // hoverOpenTimeout is 200 by default
    t = await page.evaluate(() => ({ ...t, html: testEl.outerHTML, isOpened: testEl.$isOpened }));
    expect(t.isOpened).toBe(true);
    ...
    await waitTimeout(500 + 300 + 100); // hoverOpenTimeout is 500 by default and hide-animation is 300
    t = await page.evaluate(() => ({ ...t, html: testEl.outerHTML, isOpened: testEl.$isOpened }));
    expect(t.isOpened).toBe(false);
  ...
  test("options.minWidthByTarget/minHeightByTarget", async () => {
    ...
    await page.click("label");
    await waitTimeout(10); // timeout required because of debounceFilters
