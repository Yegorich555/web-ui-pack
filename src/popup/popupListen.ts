...
export default function popupListen(
  ...
  async function show(showCase: ShowCases, e?: MouseEvent | FocusEvent): Promise<void> {
  ...
  async function hide(hideCase: HideCases, e?: MouseEvent | FocusEvent | null): Promise<void> {
    ...
    onShowEvent(document, "click", (e) => {
      ...
      if (!isTarget) {
        ...
        if (isMeClick) {
          ...
          hide(HideCases.onPopupClick, e);
        } else {
          hide(HideCases.onOutsideClick, e);
    ...
    appendEvent(t, "click", (e) => {
      ...
      if (!skip) {
        if (!openedEl) {
          ...
          show(ShowCases.onClick, e);
        } else {
          const isMeClick = openedEl === e.target || includes(e.target);
          hide(isMeClick ? HideCases.onPopupClick : HideCases.onTargetClick, e);
        }
      }
