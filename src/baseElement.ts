...
> extends HTMLElement {
  ...
  /** Child nodes created by the component itself (during the render); @see {@link WUPBaseElement.cloneNode} */
  #renderedNodes?: Set<ChildNode>;
  ...
  protected connectedCallback(): void {
    ...
    if (this.#isFirstConn) {
      ...
      const wasNodes = this.firstChild ? new Set(this.childNodes) : null; // store to detect nodes created by render
      this.gotRender();
      this.childNodes.forEach((n) => {
        if (!wasNodes?.has(n)) {
          (this.#renderedNodes ??= new Set()).add(n);
        }
      });
    }
  }

  /** Returns true if child node is created by the component itself (and false if it's defined by user).
   * Such nodes are skipped on {@link WUPBaseElement.cloneNode} */
  protected isRenderedNode(node: Node): boolean {
    if (this.#renderedNodes?.has(node as ChildNode)) {
      return true;
    }
    // WARN: by convention every generated element is stored in `$ref...` property (some of them are appended after the render: on open, on invalid etc.)
    return Object.keys(this).some((k) => k.startsWith("$ref") && this[k as keyof this] === node);
  }

  /** Returns copy of the element without content generated by the component itself
   * (otherwise cloning of rendered element produces duplicates of internal parts)
   * @tutorial Troubleshooting
   * * As for any other HTMLElement only attributes are copied: `$options` assigned via JS are missed in the clone
   * * State-attributes assigned by the component itself (`open`, `invalid`, `aria-...` etc.) are copied as well: remove it manually if the clone must be in the initial state */
  cloneNode(deep?: boolean): Node {
    const el = super.cloneNode(false); // WARN: all attributes are copied here
    if (deep) {
      this.childNodes.forEach((n) => !this.isRenderedNode(n) && el.appendChild(n.cloneNode(true)));
    }
    return el;
  }
